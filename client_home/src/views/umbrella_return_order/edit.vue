<template>
	<div class="diy_edit page_umbrella_return_order" id="umbrella_return_order_edit">
		<div class='warp'>
			<div class='container'>
				<div class='row diy_edit_content_box'>
					...
					<div v-if="$check_field('set','return_date_') || $check_field('add','return_date_') || $check_field('get','return_date_')" class="form-item col-12 col-md-6">
						<div class="diy_title">
							<span>归还日期:
							</span>
						</div>
						...
						<div class="diy_field diy_date">
							<input type="date" :disabled="disabledObj['return_date__isDisabled']" id="form_return_date_" v-model="form['return_date_']" placeholder="请输入归还日期" v-if="(form['return_date_'] && $check_field('set','return_date_')) || (!form['return_date_'] && $check_field('add','return_date_'))" />
							<span v-else-if="$check_field('get','return_date_')">{{ form[''] }}</span>
						</div>
					</div>
				</div>
	...
	export default {
		...
		methods: {
			...
			change_file(files, str) {
				...
				this.$post("~/api/umbrella_return_order/upload?", form, (res) => {
					if (res.result) {
        ...
        if (JSON.stringify(this.form["return_date_"]).indexOf("-")===-1) {
