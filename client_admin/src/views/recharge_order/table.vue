<template>
	<el-main class="bg table_wrap">
			...
			<el-row>
				...
				<el-col :xs="24" :sm="24" :lg="8" class="el_form_search_wrap">
					<el-form-item label="充值日期">
						<el-input v-model="query.recharge_date"></el-input>
					</el-form-item>
				</el-col>
				<el-col :xs="24" :sm="10" :lg="8" class="search_btn_wrap_1">
					<el-form-item>
						...
						<router-link v-if="user_group == '管理员' || $check_action('/recharge_order/table','add') || $check_action('/recharge_order/view','add')" class="el-button el-button--default el-button--primary search_btn_add" to="./view?">添加
						</router-link>
            <el-button v-if="user_group == '管理员' || $check_action('/recharge_order/table','del') || $check_action('/recharge_order/view','del')" class="search_btn_del" type="danger" @click="delInfo()">删除</el-button>
					</el-form-item>
				</el-col>
		...
		<el-table :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%" id="dataTable">
			...
			<el-table-column prop="recharge_limit" label="充值额度"
				v-if="user_group == '管理员' || $check_field('get','recharge_limit')" min-width="200">
			</el-table-column>
			...
			<el-table-column fixed="right" label="操作" min-width="120" v-if="user_group == '管理员' || $check_action('/recharge_order/table','set') || $check_action('/recharge_order/view','set') || $check_action('/recharge_order/view','get') || $check_action('//table','add') || $check_action('//view','add')" >


				<template slot-scope="scope">
					<router-link class="el-button el-button--small is-plain el-button--success" style="margin: 5px !important;"
					v-if="user_group == '管理员' || $check_action('/recharge_order/table','set') || $check_action('/recharge_order/view','set') || $check_action('/recharge_order/view','get')"
						:to="'./view?' + field + '=' + scope.row[field]"
						 size="small">
						<span>详情</span>
					</router-link>
					<el-button class="el-button el-button--small is-plain el-button--default" style="margin: 5px !important;" size="small" @click="openPayModal(scope.row)" v-if="scope.row.pay_state==='未支付' && (user_group == '管理员' || $check_pay('/recharge_order/table'))">
						<span>支付</span>
					</el-button>
				</template>
			</el-table-column>
	...
	export default {
		...
		data() {
			return {
				...
				showModal: false,
				// 获取数据地址
				url_get_list: "~/api/recharge_order/get_list?like=0",
				url_del: "~/api/recharge_order/del?",
		...
		methods: {
			...
			openPayModal(v){
				this.pay_obj.id = v.recharge_order_id
				this.payModalVisible = true
			},
			submitPay(){
				let _this = this;
				let url = this.$toUrl(this.query, "~/api/recharge_order/set?recharge_order_id="+this.pay_obj.id);
