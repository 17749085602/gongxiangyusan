	...
	<el-main class="bg edit_wrap">
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
			<el-col v-if="user_group === '管理员' || $check_field('get','order_number') || $check_field('add','order_number') || $check_field('set','order_number')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="订单编号" prop="order_number">
					<el-input id="order_number" v-model="form['order_number']" placeholder="请输入订单编号"
							  v-if="user_group === '管理员' || (form['umbrella_borrowing_order_id'] && $check_field('set','order_number')) || (!form['umbrella_borrowing_order_id'] && $check_field('add','order_number'))" :disabled="true"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','user') || $check_field('add','user') || $check_field('set','user')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="用户" prop="user">
						<div v-if="user_group !== '管理员'">
							...
							<!--v-if="user_group === '管理员' || (form['umbrella_borrowing_order_id'] && $check_field('set','user')) || (!form['umbrella_borrowing_order_id'] && $check_field('add','user'))" :disabled="disabledObj['user_isDisabled']"></el-input>-->
							<!--<div v-else-if="$check_field('get','user')">{{form['user']}}</div>-->
							<el-select v-if="user_group === '管理员' || (form['umbrella_borrowing_order_id'] && $check_field('set','user')) || (!form['umbrella_borrowing_order_id'] && $check_field('add','user'))" id="user" v-model="form['user']" :disabled="disabledObj['user_isDisabled']">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','service_fee') || $check_field('add','service_fee') || $check_field('set','service_fee')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="服务费用" prop="service_fee">
					<el-input-number id="service_fee" v-model.number="form['service_fee']"
						v-if="user_group === '管理员' || (form['umbrella_borrowing_order_id'] && $check_field('set','service_fee')) || (!form['umbrella_borrowing_order_id'] && $check_field('add','service_fee'))" :disabled="disabledObj['service_fee_isDisabled']"></el-input-number>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','lease_date') || $check_field('add','lease_date') || $check_field('set','lease_date')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="租借日期" prop="lease_date">
					<el-date-picker :disabled="disabledObj['lease_date_isDisabled']" v-if="user_group === '管理员' || (form['umbrella_borrowing_order_id'] && $check_field('set','lease_date')) || (!form['umbrella_borrowing_order_id'] && $check_field('add','lease_date'))" id="lease_date"
	...
	export default {
		...
		data() {
			return {
				...
				url_upload: "~/api/umbrella_borrowing_order/upload?",
				...
				form: {
					...
					"service_fee":0, // 服务费用
					"lease_date":'', // 租借日期
					"umbrella_borrowing_order_id": 0, // ID
        ...
        if(this.form["lease_date"]=="0000-00-00"){
          this.form["lease_date"] = null;
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/umbrella_borrowing_order/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/umbrella_borrowing_order/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/umbrella_borrowing_order/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/umbrella_borrowing_order/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/umbrella_borrowing_order/view','get');
